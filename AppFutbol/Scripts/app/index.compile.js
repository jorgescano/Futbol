...
viewManagement.index = {
    ...
    showFullPlayerList: function showFullPlayerList() {
        var areaMng = viewManagement.common;
        if (areaMng.isNullOrEmpty(areaMng.dataBase)) return;

        var players = areaMng.getPlayers();
        if (areaMng.isNullOrEmpty(players)) return;
        ...
        viewManagement.common.sortByProperty(players, "Name").forEach(function (v, i, a) {
            ...
            // Name
            cardTitle.append($('<span/>', { style: 'font-size: 1.25rem; font-weight: 500;', text: v.Name }));

            // Goalkeeper
            if (v.Goalkeeper) {
                cardTitle.append($('<span/>', { "class": 'ml-2 text-muted', title: "Suele jugar de portero" }).append($('<i/>', { "class": 'fas fa-mitten' })));
            }

            // Goals
            stats.append($('<span/>', { "class": 'mr-2' }).append($('<i/>', { "class": 'fas fa-futbol mr-1' })).append($('<span/>', { text: v.Goals })));

            // Assists
            stats.append($('<span/>', { "class": 'mr-2' }).append($('<i/>', { "class": 'fas fa-a mr-1' })).append($('<span/>', { text: v.Assists })));

            // Penalties saved
            if (!areaMng.isNullOrEmpty(v.PenaltiesSaved)) {
                stats.append($('<span/>', { "class": 'mr-2' }).append($('<i/>', { "class": 'fas fa-mitten mr-1' })).append($('<span/>', { text: v.PenaltiesSaved })));
            }

            // MVPs
            stats.append($('<span/>').append($('<i/>', { "class": 'fas fa-star mr-1' })).append($('<span/>', { text: v.MVPs })));

            // Matches
            var wins = $('<span/>').append($('<span/>', { text: 'V', "class": 'square win' })).append($('<span/>', { text: v.Wins, "class": 'square value' }));
            var draws = $('<span/>').append($('<span/>', { text: 'E', "class": 'square draw' })).append($('<span/>', { text: v.Draws, "class": 'square value' }));
            var defeats = $('<span/>').append($('<span/>', { text: 'D', "class": 'square defeat' })).append($('<span/>', { text: v.Defeats, "class": 'square value' }));
    ...
    showStatPlayerList: function showStatPlayerList(statName, averageStatName) {
        if (viewManagement.common.isNullOrEmpty(viewManagement.common.dataBase)) return;

        if (viewManagement.common.dataBase.Players.filter(function (e) {
            return !viewManagement.common.isNullOrEmpty(e[statName]);
        }).length == 0) return;
        ...
        players.forEach(function (v, i, a) {
            ...
            // Matches
            var info = $('<div/>');
            var played = $('<span/>').append($('<span/>', { text: 'PJ', "class": 'square stat' })).append($('<span/>', { text: v.Matches, "class": 'square stat-value' }));
            info.append(played);
            if (!viewManagement.common.isNullOrEmpty(v[averageStatName])) {
                var averageStat = $('<span/>', { "class": 'square stat' });
                if (statName == "Goals") averageStat.append($('<i/>', { "class": 'fas fa-futbol', style: 'font-size: 0.9rem;' }));else if (statName == "Assists") averageStat.append($('<span/>', { text: 'A' }));
                averageStat.append($('<span/>', { text: '/P' }));
                var average = $('<span/>').append(averageStat).append($('<span/>', { text: v[averageStatName], "class": 'square stat-value' }));
                info.append(average);
            }
